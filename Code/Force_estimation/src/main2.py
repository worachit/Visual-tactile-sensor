...
class VisualTactile:
    def __init__(self):
        ...
        self.n_init_frame = 0
        self.N_init_frame = 100
        self.is_set_init_frame = True
    ...
    def setInitFrame(self):
        if self.is_set_init_frame:

            if (self.init_frame is None):
                self.init_frame = self.processing_frame
            else:
                self.init_frame = (self.n_init_frame*self.init_frame + self.processing_frame)/(self.n_init_frame+1)
            
            if self.n_init_frame > self.N_init_frame:
                self.is_set_init_frame = False
            else:
                self.n_init_frame += 1

    def getDepthMap(self):
        if self.is_set_init_frame: 
            return self.init_frame.astype(np.uint8)
        else:
            return np.abs(self.processing_frame.astype(float) - self.init_frame).astype(np.uint8)
    ...
    def captureImage(self):
        ...
        while True:
            ...
            self.setInitFrame()
            self.processing_frame = self.getDepthMap()
